...
#define LASSERT_TYPE(func, args, index, expected) \
  LASSERT(args, args->cell[index]->type == expected, \
      "Function '%s' passed incorect type for argument %i. " \
      "Got %s, Expected %s.", \
...
lval* lval_eval_sexpr(lenv* e, lval* v) {
  ...
  //first elem is a function after evaluation?
  lval* f = lval_pop(v, 0);
  if (f->type != LVAL_FUN) {
    lval* err = lval_err(
...
lval* lval_pop(lval* v, int idx) {
  lval* x = v->cell[idx];
  //shift memory after the item at idx over the top
  memmove(&v->cell[idx], &v->cell[idx+1],
      sizeof(lval*) * (v->count-idx-1));
...
lval* builtin_head(lenv* e, lval* a) {
  ...
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR,
      "Function 'head' passed incorrect type for arugment 0. "
      "Got %s, Expected %s.",
...
lval* builtin_tail(lenv* e, lval* a) {
  LASSERT(a, a->count == 1,
      "Function 'tail' too many arugments. "
      "Got %i, Expected %i.", a->count, 1);
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR,
...
lval* builtin_var(lenv* e, lval* a) {
  ...
  for (int i = 0; i < syms->count; i++) {
    LASSERT(a, syms->cell[i]->type == LVAL_SYM,
        "Function 'def' cannot define non-symbol type %s",
        ltype_name(syms->cell[i]->type));
  }
...
int main(int argc, char** argv) {
  ...
  mpca_lang(MPCA_LANG_DEFAULT,
        ...
        jerispy : /^/ <expr>* /$/ ; \
      ",
      Number, Symbol, Sexpr, Qexpr, Expr, Jerispy);
  ...
  //MPC cleanup
  mpc_cleanup(4, Number, Symbol, Sexpr, Qexpr, Expr, Jerispy);
  return 0;
}
